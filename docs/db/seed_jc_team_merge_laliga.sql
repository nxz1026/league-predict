...
-- ============================================================================
-- ✅ **已执行 2026-09-16 23:27**（用户批复「执行」，COMMIT 成功）
--    执行方式：cp 到 /tmp 后 sudo -n -u postgres psql -d league -f
--    （postgres 用户对 /home/ubuntu 无读权限，必须经 /tmp）
--    执行后重跑 bash ~/bin/jc-ingest-run.sh → 西甲 n_af_ready 3 → 5，
--    171→3114 / 172→2354 指向 winner，孤儿行 120506/120507 零引用，
--    jc_match 引用孤儿 = 0，457 passed 无回归。
--
-- ⚠️ 重跑本文件是幂等的（先摘后加，四步都带幂等条件）；
--    前置断言在 winner 已持键时会 fail fast，属于预期行为（说明已执行过）。
--
