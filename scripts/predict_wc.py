...
P3: Onside 4 信号模型 + 蒙特卡洛赛季/杯赛模拟 + 多联赛支持

v4.0 改进（2026-07-20）：
- 移除世界杯特化逻辑，统一为联赛模式
- 默认数据源切为 football-data.org
- 默认联赛切为 epl

用法: python3 predict_wc.py [--dates YYYYMMDD-YYYYMMDD] [--no-fetch] [--cleanup]
      --no-fetch: 使用本地 /tmp/espn_league.json (调试用)
      --cleanup: 清理超过7天的 predictions/ 和 results/ 文件
      --league=epl|laliga|bundesliga|seriea|ligue1: 联赛选择
      --data-source=football-data|espn|api-football: 数据源选择
...
ESPN_URL_TEMPLATE = "https://site.api.espn.com/apis/site/v2/sports/soccer/{league_slug}/scoreboard?dates={dates}&limit=50"
...
def fetch_espn(dates_str, league_slug="epl"):
    ...
    for attempt in range(1, ESPN_MAX_RETRIES + 1):
        try:
            ...
            # 存一份到 /tmp 供调试
            with open("/tmp/espn_league.json", "w") as f:
